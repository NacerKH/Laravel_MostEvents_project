    ...
    <div class="row mt-3">
        <div class="col-md-3">
            <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                ...
                <a class="nav-link " id="v-pills-menu-tab" data-toggle="pill" href="#v-pills-menu" role="tab" aria-controls="v-pills-menu" aria-selected="true"><i class="fas fa-list mr-2"></i> Events</a>
                <a class="nav-link " id="v-pills-book-tab" data-toggle="pill" href="#v-pills-book" role="tab" aria-controls="v-pills-book" aria-selected="true"><i class="fas fa-archive mr-2"></i> Reservations of Spaces Commercial</a>

                <a class="nav-link" id="v-pills-settings-tab" href="{{route('CreatorEvents.edit',$cevent->id)}}"><i class="fas fa-cog mr-2"></i> Settings</a>
            </div>
        </div>
                        ...
                        <tbody>
                          ...
                          @foreach($cevent->bookingt() as $item)
                            ...
                            <td>         
                             ...
                             <div class="btn-group">
                                             @if($item->validate == 0)
                                                        <form id="status-form-{{ $item->id }}" action="{{ route('approve-book',$item->id) }}" style="display: none;" method="POST">
                                                            @csrf
                                                            </form>
                                     ...
                                     <form action="{{ route('bookingticket.destroy',[ $item->id])}}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
        ...
        <div class="tab-pane fade " id="v-pills-menu" role="tabpanel" aria-labelledby="v-pills-menu-tab"> 
                   ...
                   <div class="col-12 row my-2 " style="{{($item->active)?'':'opacity: 0.5;'}}">
                       <div class="col-3"> 
                       ...
                       <div class="col-9">
                           ...
                           <p class="text-danger">Date Event</p><p class="text-info">{{$item->date}} From:{{$item->from}} To:{{$item->to}}</p>

                           <hr>
                           <div class="btn-group">
                                   <a href="{{route('Events.edit',$item->id)}}" class="btn btn-sm btn-primary">Edit</a>
                                   @if($item->active == false)
                                   <a href="{{route('Events.edit',$item->id)}}?action=enable" class="btn btn-sm btn-success">Enable</a>
                                   @else
                                   <a href="{{route('Events.edit',$item->id)}}?action=disable" class="btn btn-sm btn-light">Disable</a>
                                   @endif
                        ...
                        @foreach($bookss as $book)
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <img src="{{$book->namelogo()}}" alt="{{$book->namelogo()}}" class="avatar">
                                </td>
                                <td>{{$book->nameoner()}}</td>
                                <td>{{$book->date}}</td>
                                <td>{{$book->from}}</td>
